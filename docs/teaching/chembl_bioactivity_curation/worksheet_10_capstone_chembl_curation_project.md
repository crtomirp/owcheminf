# Worksheet 10: Capstone: Curate a ChEMBL Dataset for QSAR

## Context

Students complete a mini-project from target selection to curated modelling table.
...
```text
ChEMBL Browser → ChEMBL Bioactivity Retriever → Mol Standardizer → QSAR workflow
```
...
Select one target, retrieve records, curate a dataset, build a simple model, and report limitations.
...
Is the final model limited by chemistry, assay heterogeneity, dataset size, or applicability domain?
