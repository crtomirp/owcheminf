# Worksheet 08: Building a QSAR-Ready pChEMBL Dataset

## Context

Students create a modelling table with one target variable and standardized structures.
...
```text
Mol Standardizer → Mol Descriptors 2 / Fingerprint Generator → QSAR Regression
```
...
Build a clean dataset with one row per molecule and a defined target variable.
...
What makes a dataset QSAR-ready? Which columns should remain as metadata?
