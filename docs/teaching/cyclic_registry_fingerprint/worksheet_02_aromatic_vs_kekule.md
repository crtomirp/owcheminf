# Worksheet 02 — Aromatic versus kekulized representations

## Context

Students compare aromatic and kekulized SMILES forms and evaluate whether the fingerprint behaves robustly.

**Keywords:** SMILES representation, aromaticity, kekulization
...
```text
File → Cyclic Registry Fingerprint → Data Table; Matched Registry Entries → Data Table
```
...
owcheminf-cyclic-registry-fingerprint \
  ...
  --name-column name \
  --out-prefix outputs/worksheet_02_aromatic_vs_kekule_crfp \
  --write-json
```
...
```text
outputs/worksheet_02_aromatic_vs_kekule_crfp.active_bits.csv
outputs/worksheet_02_aromatic_vs_kekule_crfp.matches.csv
outputs/worksheet_02_aromatic_vs_kekule_crfp.summary.json
```
...
- Do pyridine and pyridine_kekule activate similar registry entries?
- Why can databases store equivalent structures differently?
- Which parts of the fingerprint are representation-sensitive?
- How could standardization reduce representation artifacts?
