# Worksheet 07 — Profiling cyclic motifs among top docking candidates

## Context

Students analyze whether top-ranked docking hits share interpretable cyclic motifs.

**Keywords:** docking, virtual screening, hit triage
...
```text
File → Select Rows/top hits → Cyclic Registry Fingerprint → Matched Registry Entries
```
...
owcheminf-cyclic-registry-fingerprint \
  ...
  --name-column name \
  --out-prefix outputs/worksheet_07_docking_results_crfp \
  --write-json
```
...
```text
outputs/worksheet_07_docking_results_crfp.active_bits.csv
outputs/worksheet_07_docking_results_crfp.matches.csv
outputs/worksheet_07_docking_results_crfp.summary.json
```
...
- Which motifs occur in the top-ranked candidates?
- Are the same motifs present across multiple targets?
- Do motif frequencies differ between top and low-ranked compounds?
- Which compounds should be prioritized for visual inspection?
