# Worksheet 09 — Learning SMARTS through registry matches

## Context

Students use matched entries as examples to learn how SMARTS patterns represent chemical substructures.

**Keywords:** SMARTS, substructure search, cheminformatics education
...
owcheminf-cyclic-registry-fingerprint \
  ...
  --name-column name \
  --out-prefix outputs/worksheet_09_smarts_learning_crfp \
  --write-json
```
...
```text
outputs/worksheet_09_smarts_learning_crfp.active_bits.csv
outputs/worksheet_09_smarts_learning_crfp.matches.csv
outputs/worksheet_09_smarts_learning_crfp.summary.json
```
...
- Why can one molecule match several registry patterns?
- How do atom match indices help verify a match?
- How would you design a SMARTS pattern for oxazole or thiazole?
