# Worksheet 05 — Interpreting important model features

## Context

Students connect high-ranked model features to chemically meaningful registry entries.

**Keywords:** feature importance, interpretability, matched entries
...
```text
File → Cyclic Registry Fingerprint → Rank; Matched Registry Entries → Data Table
```
...
owcheminf-cyclic-registry-fingerprint \
  ...
  --name-column name \
  --out-prefix outputs/worksheet_05_feature_interpretation_crfp \
  --write-json
```
...
```text
outputs/worksheet_05_feature_interpretation_crfp.active_bits.csv
outputs/worksheet_05_feature_interpretation_crfp.matches.csv
outputs/worksheet_05_feature_interpretation_crfp.summary.json
```
...
- Which active bits are most associated with the endpoint?
- Can important bits be mapped to named heterocycles or functional groups?
- Which important bits are hashed Morgan bits and therefore less interpretable?
- How would you report an interpretable feature in a paper?
