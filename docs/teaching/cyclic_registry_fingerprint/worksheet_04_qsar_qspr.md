# Worksheet 04 — Using the fingerprint in QSAR/QSPR modeling

## Context

Students use the 4096-bit fingerprint as model input and compare it with generic fingerprints.

**Keywords:** QSAR, QSPR, machine learning, descriptors
...
```text
File → Cyclic Registry Fingerprint → Test & Score / Random Forest / Rank
```
...
owcheminf-cyclic-registry-fingerprint \
  ...
  --name-column name \
  --out-prefix outputs/worksheet_04_qsar_qspr_crfp \
  --write-json
```
...
```text
outputs/worksheet_04_qsar_qspr_crfp.active_bits.csv
outputs/worksheet_04_qsar_qspr_crfp.matches.csv
outputs/worksheet_04_qsar_qspr_crfp.summary.json
```
...
- Does the registry-enhanced fingerprint improve model performance?
- Which bit ranges contribute most?
- Is higher dimensionality always beneficial?
