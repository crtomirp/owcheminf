# Worksheet 06 — Comparing compound libraries by cyclic motif profile

## Context

Students compare two or more libraries using motif frequencies rather than only global similarity.

**Keywords:** library design, screening, diversity, motif enrichment
...
owcheminf-cyclic-registry-fingerprint \
  ...
  --name-column name \
  --out-prefix outputs/worksheet_06_library_comparison_crfp \
  --write-json
```
...
```text
outputs/worksheet_06_library_comparison_crfp.active_bits.csv
outputs/worksheet_06_library_comparison_crfp.matches.csv
outputs/worksheet_06_library_comparison_crfp.summary.json
```
...
- Which library is richer in heterocycles?
- Which library is more carbocycle-dominated?
- Which motifs are unique to one library?
- How could motif profiles guide screening-library selection?
