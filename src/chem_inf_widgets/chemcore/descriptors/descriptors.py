...
def compute_fingerprints_from_smiles(
    ...
    *,
    ...
    """
    Pure computation:
    - parses SMILES -> RDKit mol
    - computes bit fingerprints -> X (0/1)
    - drops failed molecules
    - optional low-variance filtering

    Callbacks:
      progress_cb(pct:int) where pct in [0..100]
      cancel_cb() -> bool (True means stop early)
    """
    ...
    for i, smi in enumerate(smiles_list):
        ...
        mol = _mol_from_smiles(str(smi), sanitize=sanitize, remove_hs=remove_hs)

        fp_arr: Optional[np.ndarray]
        if mol is None:
            fp_arr = None
        else:
            try:
                fp_arr = _compute_fp(mol, fp_type, bit_size=bit_size, radius=radius)
            except Exception:
                fp_arr = None
    ...
    X = np.vstack(rows).astype(np.float32)  # (n_valid, n_bits)

    bit_names = [f"{fp_type}_{j}" for j in range(X.shape[1])]
    ...
    if remove_low_variance and X.shape[1] > 0:
        ...
        if np.any(keep):
            ...
            # keep at least 1 column to avoid empty table surprises
            X = X[:, :1]
